...
class Console:
    ...
    def update_title() -> None:
        ...
        while True:
            ...
            os.system(
                f"title  Done: {activated_accounts}"
            )
            thread_lock.release()
...
class Nitro:
    ...
    def activate_nitro(self):
        ...
        try:
            ...
            if (
                "looks like this code has already been used."
                in self.driver.page_source
            ):
                Console().sprint("Code has been Claimed", False)
                thread_lock.acquire()
                with open("input/promos.txt", "r+") as io:
                    tokens = io.readlines()
                    io.seek(0)
                    for line in tokens:
                ...
                thread_lock.acquire()
                with open("input/tokens.txt", "r+") as io:
                    tokens = io.readlines()
                    io.seek(0)
                    for line in tokens:
        ...
        if not ("Today's Total" in self.driver.page_source):
            for ___ in range(2):
                ...
                try:
                    ...
                    with open("input/Success.txt", "a") as nitro_success:
                    ...
                    if ___ != 2:
                        ...
                        sys.stdout.write(
                            f"{Fore.RED}Error Activating Nitro | {self.token}\n"
                        )
            ...
            try:
                ...
                Console().sprint(f"Nitro Succsesfully Activated | {self.token}", True)
                ...
                Console().sprint(f"Error Activating Nitro | {self.token}", False)
    ...
    def __main__(self):
        ...
        try:
            ...
            Console().sprint("Proxy is Bad", False)
        except:
            Console().sprint("Error", False)


if __name__ == "__main__":
    Console().clear()

    promo_links = open("input/promos.txt", "r").read().splitlines()
    tokens = open("input/tokens.txt", "r").read().splitlines()
    vcc = open("input/vcc.txt", "r").read().splitlines()
    proxies = open("input/proxies.txt", "r").read().splitlines()


    print("Kann Irgendwie Kein fonts rein Packen deshalb Einfach\n\nTools Town\n")

    sys.stdout.write(
        f"{Fore.MAGENTA}Promos: {len(promo_links)}\nTokens: {len(tokens)}\nVcc's: {len(vcc)}\nProxies: {len(proxies)}\n\n"
    )

    new_list = []

    with open("input/config.json", "r") as jsonfile:
    ...
    def delete_vcc():
        while not check_status:
            time.sleep(10)
            if len(used_vcc) <= 0:
                with open("input/vcc.txt", "r+") as io:
                    vccs = io.readlines()
                    io.seek(0)
                    for line in vccs:
    ...
    try:
        while len(tokens) and len(new_list) and len(promo_links) >= 1:
            local_threads = []
            ...
            for _ in range(int(thread_count)):
                ...
                try:
                    ...
                    start_thread = threading.Thread(
                        target=Nitro(tokens[0], cc, promo_links[0], proxy).__main__
                    )
    ...
    sys.stdout.write(f"{Fore.WHITE}Fist you Need to import some promos and Tokens\n")
